...
import { FiClock, FiFile, FiDownload, FiTrash2, FiAlertCircle, FiTrash } from 'react-icons/fi';
...
function History() {
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${darkMode ? 'bg-[#1a1f2e]' : 'bg-gray-100'}`}>
      <div className={`absolute inset-0 -z-10 h-full w-full ${darkMode ? 'bg-slate-950' : 'bg-white'} bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px]`}></div>
        ...
        <div className="animate-fade-in">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8">
            <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-0">
              <FiClock className={`w-5 h-5 sm:w-6 sm:h-6 transition-colors duration-300 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
              <h1 className={`text-xl sm:text-2xl font-bold transition-colors duration-300 ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                Conversion History
              </h1>
            </div>
            {history.length > 0 && (
          ...
          <div className={`rounded-2xl transition-all duration-300 border shadow-lg hover:shadow-xl ${
            darkMode ? 'bg-[#1f2433]/70 border-gray-700/50 backdrop-blur-md' : 'bg-white/70 border-gray-200 backdrop-blur-md'
          } p-4 sm:p-6`}>
            {history.length > 0 ? (
                ...
                {history.map((item) => (
                  ...
                  >
                    <div className="flex items-center space-x-3 sm:space-x-4 mb-3 sm:mb-0 flex-grow w-full sm:w-auto min-w-0">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex-shrink-0 flex items-center justify-center transition-colors duration-300 ${darkMode ? 'bg-blue-500/10' : 'bg-blue-100'}`}>
                        <FiFile className={`w-5 h-5 transition-colors duration-300 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                      </div>
                      <div className="flex-grow min-w-0">
                        <h3 className={`font-medium text-sm sm:text-base break-words transition-colors duration-300 ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                          {item.fileName}
                        </h3>
                        <div className={`text-xs sm:text-sm transition-colors duration-300 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                          <span className="block sm:inline">{format(new Date(item.date), 'MMM d, yyyy h:mm a')}</span>
                          <span className="hidden sm:inline mx-1 sm:mx-2">•</span>
                          <span className="block sm:inline">{formatFileSize(item.originalSize)} → {formatFileSize(item.convertedSize)}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2 self-end sm:self-center flex-shrink-0 mt-2 sm:mt-0">
                      <button
                        onClick={() => handleDownload(item.pdfData, item.convertedFileName)}
              ...
              <div className="text-center py-10 sm:py-16">
                <FiClock className={`w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 transition-colors duration-300 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} />
                <h3 className={`text-base sm:text-lg font-medium mb-2 transition-colors duration-300 ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] px-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity duration-300 animate-fade-in" 
               onClick={() => setShowDeleteModal(false)} />
          <div className={`relative rounded-2xl shadow-xl transition-all duration-300 transform border animate-modal-pop ${
            darkMode ? 'bg-[#1f2433] border-gray-700' : 'bg-white border-gray-200'
          } p-5 sm:p-6 max-w-sm sm:max-w-md w-full`}>
            <div className="flex items-start mb-4">
              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex-shrink-0 flex items-center justify-center mr-3 sm:mr-4 transition-colors duration-300 ${darkMode ? 'bg-red-500/10' : 'bg-red-100'}`}>
                <FiAlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-red-500" />
              </div>
              <div className="flex-grow">
                <h3 className={`text-base sm:text-lg font-semibold mb-1 sm:mb-2 transition-colors duration-300 ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                  Delete File
                </h3>
                <p className={`text-xs sm:text-sm transition-colors duration-300 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Are you sure you want to delete "<span className="font-medium break-all">{selectedItem?.fileName}</span>"? This action cannot be undone.
                </p>
              </div>
            </div>
