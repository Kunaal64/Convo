...
import { FiHome, FiClock, FiSettings, FiSun, FiMoon, FiMenu, FiX } from "react-icons/fi";
...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${
          ...
          <div className="flex items-center">
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="relative">
                ...
                <div className={`absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full ring-2 transition-all duration-300 ${darkMode ? 'ring-gray-900' : 'ring-white'}`}></div>
              </div>
              <div className="flex items-baseline">
                <span className={`font-bold text-2xl bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 bg-clip-text text-transparent transition-all duration-300 group-hover:opacity-90`}>
                  Convo
                </span>
                <span className={`hidden sm:block text-xs ml-1.5 font-medium tracking-wider uppercase transition-colors duration-300 ${
      ...
      {/* Mobile menu */}
      <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:hidden border-t ${darkMode ? 'border-gray-700/50' : 'border-gray-200'}`} id="mobile-menu">
